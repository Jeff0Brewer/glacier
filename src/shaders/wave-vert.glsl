...
void main() {
    ...
    float height = heightMap(surfaceMap, texCoord, heightScale) + 0.1;
    vec3 pos = vec3(position.xy, height + pow((position.z * 0.05), 3.0));
    gl_Position = projMatrix * viewMatrix * modelMatrix * scaleMatrix * vec4(pos, 1.0);

    float brightness = (1.0 - pow(speed, 0.2));
    color = vec3(brightness, brightness, 1.0);
    fade = sign(ind) * (1.0 - mod((ind - currInd), maxInd) / maxInd);
}
